...
aflags_show(struct device *dev, struct device_attribute *attr,
		     char *buf)
{
	struct vfio_sdmdev *sdmdev = vfio_sdmdev_pdev_sdmdev(dev->parent);
	struct dummy_hw *hw = (struct dummy_hw *)sdmdev->priv;
...
static int dummy_get_queue(struct vfio_sdmdev *sdmdev,
...
static void dummy_put_queue(struct vfio_sdmdev_queue *q)
{
...
static int dummy_mmap(struct vfio_sdmdev_queue *q, struct vm_area_struct *vma)
{
	struct dummy_hw_queue *hwq = (struct dummy_hw_queue *)q->priv;
...
static long dummy_ioctl(struct vfio_sdmdev_queue *q, unsigned int cmd,
				unsigned long arg)
{
	struct dummy_hw_queue *hwq = q->priv;
...
static void dummy_mask_notify(struct vfio_sdmdev_queue *q, int event_mask)
{
	pr_info("mask notify: %x\n", event_mask);
}

static const struct vfio_sdmdev_ops dummy_ops = {
...
static int dummy_wd_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MAX_QUEUE; i++) {
		...
		hw->qs[i].wdq.sdmdev = sdmdev;
		hw->qs[i].hw = hw;
	}

	platform_set_drvdata(pdev, sdmdev);
	sdmdev->iommu_type = VFIO_NOIOMMU_IOMMU;
	sdmdev->name = DUMMY_WD;
	sdmdev->dev = &pdev->dev;
	sdmdev->is_vf = 0;
	sdmdev->priv = hw;
...
{
	struct vfio_sdmdev *sdmdev = (struct vfio_sdmdev *)pdev->dev.driver_data;
	vfio_sdmdev_unregister(sdmdev);
	return 0;
}
...
static int __init dummy_vfio_sdmdev_init(void)
{
	int i, j;
...
static void __exit dummy_vfio_sdmdev_exit(void)
{
	int i;
...
module_init(dummy_vfio_sdmdev_init);
module_exit(dummy_vfio_sdmdev_exit);
