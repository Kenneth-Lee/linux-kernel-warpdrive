...
static int _pasid_op(struct vfio_spimdev_queue *q, struct int container,
		     int cmd)
{
	struct vfio_iommu_type1_bind *bind; /* support type1 only now */
	int container = arg;
	int ret;

	bind = compat_alloc_user_space(sizeof(*bind));
	if (!bind)
		goto err_with_fd;

	bind->bind.flags = VFIO_IOMMU_BIND_PROCESS;
	bind->bind.argsz = sizeof(*bind);
	bind->data.flags = q->dma_flag; /* todo: need a default value */

	if(cmd == VFIO_IOMMU_UNBIND)
		bind->data.pasid = q->pasid;

	ret = do_ioctl(container, cmd, &bind);

	if (cmd == VFIO_IOMMU_BIND && !ret)
		q->pasid = bind->data.pasid;
...
#ifdef CONFIG_IOMMU_SVA
	ret = _pasid_op(q, q->container, VFIO_IOMMU_UNBIND);
	if (ret) {
		dev_err(spimdev->dev, "VFIO_IOMMU_UNBIND fail (%d)!\n", ret);
		return ret;
	}
#endif
...
#ifdef CONFIG_IOMMU_SVA
	ret = _pasid_op(q, arg, VFIO_IOMMU_BIND);
	if (ret)
		goto err_with_fd;

	ret = spimdev->ops->set_pasid(q, q->pasid);
#endif
