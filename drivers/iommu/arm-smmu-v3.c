				       ...
				       unsigned long flags)
{
	...
	if (mm) {
		...
		entry = ops->alloc_priv_entry(ops, fmt, &cfg);
	}
...
static void arm_smmu_mm_free(struct io_mm *io_mm)
{
	...
	free_io_pgtable_ops(smmu_mm->ops);
	iommu_free_pasid_entry(smmu_mm->entry);
	kfree(smmu_mm);
}
			      ...
			      struct io_mm *io_mm, bool attach_domain)
{
	...
	return ops->set_entry(ops, io_mm->pasid, smmu_mm->entry);
}
