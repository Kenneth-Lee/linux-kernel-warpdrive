...
/* set pasid to -1 for unset it */
static int hisi_qm_set_pasid(struct vfio_spimdev_queue *q, int pasid)
{
	struct hisi_qp *qp = q->priv;

	QM_SQC(qp)->pasid = pasid;

	return _hacc_mb(qp->qm, MAILBOX_CMD_SQC,
			qp->sqc.dma, qp->queue_id, 0, 0);
}
