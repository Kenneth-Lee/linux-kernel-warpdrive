...
static int hisi_sec_queue_ar_pkgattr_set(struct wd_queue *queue, u32 ar_pkg)
{
...
static int hisi_sec_queue_aw_pkgattr_set(struct wd_queue *queue, u32 aw_pkg)
{
...
static int hisi_sec_clk_en(struct wd_dev *sec)
{
	...
	while (1) {
		...
		mdelay(1);
		if ((readl_relaxed(base + HISI_SEC_CLK_ST) & 0x7) == 0x7)
			return 0;
		i++;
		if (i > 10) {
			dev_err(sec->dev, "sec clock enable fail!\n");
			return -EIO;
		}
	}
}

static int hisi_sec_clk_dis(struct wd_dev *sec)
{
	void *base = DEV_IO_ADDR(sec, 0);
	u32 i = 0;
	...
	while (1) {
		...
		mdelay(1);
		if ((readl_relaxed(base + HISI_SEC_CLK_ST) & 0x7) == 0)
			return 0;
		i++;
		if (i > 10) {
			dev_err(sec->dev, "sec clock disable fail!\n");
			return -EIO;
		}
	}
}
...
static int hisi_sec_reset_whole_module(struct wd_dev *sec)
{
	...
	while (1) {
		...
		mdelay(1);
		if ((SEC_NOT_RESET_ST ==
		     (readl_relaxed(base + HISI_SEC_RESET_ST) & 0x1)) &&
		    (SEC_NOT_RESET_ST ==
		     (readl_relaxed(base + HISI_SEC_BUILD_RESET_ST) & 0x1)))
			break;
...
static int hisi_sec_saa_getqm_en(struct wd_dev *dev, u32 saa_indx, u32 en)
{
	...
	saa_ctrl.u32 = readl_relaxed(base + HISI_SEC_SAA_BASE +
				     HISI_SEC_SAA_CONTROL_REG +
				     saa_indx * HISI_SEC_SAA_ADDR_SIZE);
	saa_ctrl.bits.get_qm_en = (en & 0x1);
	writel_relaxed(saa_ctrl.u32,
		       base + HISI_SEC_SAA_BASE + HISI_SEC_SAA_CONTROL_REG +
		       saa_indx * HISI_SEC_SAA_ADDR_SIZE);
	return 0;
}
				...
				u32 saa_indx, u32 saa_int_mask)
{
	writel_relaxed(saa_int_mask,
		       DEV_IO_ADDR(dev, 1) + HISI_SEC_SAA_BASE +
...
static int hisi_sec_queue_swproc_ptr_set(struct wd_queue *queue, u32 value)
{
	writel_relaxed(value,
		       Q_IO_ADDR(queue, 0) + HISI_SEC_Q_SOFT_PROC_PTR_REG);
	return 0;
}
...
static int hisi_sec_queue_errbase_laddr_set(struct wd_queue *queue,
					    u32 low_addr)
{
	writel_relaxed(low_addr,
		       Q_IO_ADDR(queue, 0) +
...
{
	hisi_sec_queue_proc_int_mask_set(queue, 0xffffffff);
}

void hisi_sec_queue_irq_enable(struct wd_queue *queue)
{
	hisi_sec_queue_proc_int_mask_set(queue, 0);
}

void hisi_sec_queue_abn_irq_disable(struct wd_queue *queue)
{
	hisi_sec_queue_abnrml_int_mask_set(queue, 0xffffffff);
}

void hisi_sec_queue_abn_irq_enable(struct wd_queue *queue)
{
	hisi_sec_queue_abnrml_int_mask_set(queue, 0);
}
...
{
	hisi_sec_queue_enable(queue, 0x1);
...
void hisi_sec_response_handler(uintptr_t queue_addr)
{
	...
	hisi_sec_queue_irq_disable(queue);
	hisi_sec_poll((void *)queue_addr, 0);
	hisi_sec_queue_irq_enable(queue);
...
int hisi_sec_queue_hw_init(struct wd_queue *queue)
{
	...
	hisi_sec_queue_ar_alloc_set(queue, 1);
	hisi_sec_queue_aw_alloc_set(queue, 1);
	hisi_sec_queue_ar_pkgattr_set(queue, 1);
	hisi_sec_queue_aw_pkgattr_set(queue, 1);

	/*disable reorder queue, as this is set 0*/
	hisi_sec_queue_reorder_set(queue, 1);

	hisi_sec_queue_procnum_cfg(queue, 1);
	hisi_sec_queue_depth_set(queue, qinfo->ring[SEC_CMD_RING].depth - 1);

	hisi_sec_queue_cmdbase_haddr_set(queue,
		(((u64)Q_IO_IOVA(queue, SEC_CMD_RING)) >> 32) & 0xFFFFFFFF);
	hisi_sec_queue_cmdbase_laddr_set(queue,
		((u64)Q_IO_IOVA(queue, SEC_CMD_RING)) & 0xFFFFFFFF);
	hisi_sec_queue_outorder_haddr_set(queue,
		(((u64)Q_IO_IOVA(queue, SEC_OUTORDER_RING)) >> 32) &
		0xFFFFFFFF);
	hisi_sec_queue_outorder_laddr_set(queue,
		((u64)Q_IO_IOVA(queue, SEC_OUTORDER_RING)) & 0xFFFFFFFF);
	hisi_sec_queue_errbase_haddr_set(queue,
		(((u64)Q_IO_IOVA(queue, SEC_DBG_RING)) >> 32) & 0xFFFFFFFF);
	hisi_sec_queue_errbase_laddr_set(queue,
		((u64)Q_IO_IOVA(queue, SEC_DBG_RING)) & 0xFFFFFFFF);

	hisi_sec_queue_ovrtmth_set(queue, 0xffffffff);
	hisi_sec_queue_abn_irq_disable(queue);
	hisi_sec_queue_irq_enable(queue);
	hisi_sec_queue_init_set(queue, 0x3);
...
static int hisi_sec_hw_init(struct wd_dev *dev)
{
	...
	hisi_sec_saa_clk_en(dev, 0x01f);
#else
	hisi_sec_saa_clk_en(dev, 0x3ff);
#endif

	/* 32 bit little endian */
	hisi_sec_bd_endian_set(dev, 0);
	hisi_sec_bd_endian_type_set(dev, 0);

	/* cache cfg */
	hisi_sec_cache_config_set(dev);

	/*
	 * data axi port write and read outstanding config,
	 * config as suggestion
	 */
	hisi_sec_data_axiwr_otsd_cfg(dev, 0x7);
	hisi_sec_data_axird_otsd_cfg(dev, 0x7);

	/* enable clock gating */
	hisi_sec_clk_gate_en(dev, 1);

	/* set CNT_CYC register not read clear */
	hisi_sec_comm_cnt_cfg(dev, 0);

	/* enable CNT_CYC */
	hisi_sec_commsnap_en(dev, 0);

	hisi_sec_fsm_maxcnt_set(dev, 0xffffffff);
	...
	hisi_sec_ipv6_hashmask_set(dev, sec_ipv6_mask);

	/*  not use debug bd*/
	hisi_sec_set_dbg_bd_cfg(dev, 0);

#if defined(CONFIG_ARM_SMMU_V3)
	hisi_sec_streamid_set(dev);
	hisi_sec_weight_set(dev);
#endif
	for (i = 0; i < HISI_SEC_MAX_SAA_NUM; i++) {

		/* all saa enable */
		hisi_sec_saa_getqm_en(dev, i, 1);

		/* all saa interrupt not mask*/
		hisi_sec_saa_set_int_mask(dev, i, 0);
	}
	...
	hisi_sec_create_wd_dev(struct platform_device *pdev)
{
	...
	if (!of_device_is_compatible(np, "hisilicon,hip07-sec")) {
...
static void hisi_sec_base_exit(struct wd_dev *sec)
{
	hisi_sec_clk_dis(sec);
	hisi_sec_unmap_io(sec);
}
...
static int hisi_sec_capability_init(struct wd_dev *dev)
{
	...
	dev_info->capa[WD_AT_CY_AUTH] =
		(struct _sec_wd_capa **)sec_dgst_capa_tbl;
...
static int hisi_sec_queue_res_cfg(struct wd_queue *queue)
{
	...
	info->ring[SEC_OUTORDER_RING].base =
		Q_IO_ADDR(queue, SEC_OUTORDER_RING);
...
static const struct wd_dev_ops hisi_sec_ops = {
	.get_queue = _alloc_queue,
	.put_queue = _free_queue,
	.is_q_updated = sec_q_updated,
	.mask_notification = sec_mask_notification,
	.mmap = _queue_mmap,
	.open = _open_queue,
	.close = _close_queue,
	.reset_queue = _reset_queue,
	.ioctl = _queue_ioctl,
};
...
static int hisi_sec_queue_trigger(struct wd_queue *q)
{
	...
	dma_buffer = dma_map_single(dev, buffer, 512, DMA_BIDIRECTIONAL);
...
dmamap_fail:
	...
	dma_unmap_single(dev, dma_buffer, 512, DMA_BIDIRECTIONAL);
	kfree(buffer);
