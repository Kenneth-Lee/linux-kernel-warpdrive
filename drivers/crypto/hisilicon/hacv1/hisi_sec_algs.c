...
static int sec_alg_ablkcipher_crypto(struct ablkcipher_request *req, int direct)
{
	...
	memset(sec_req, 0, sizeof(struct sec_crypto_request));
	msg = &sec_req->req;
	if (direct)
		memcpy(msg, &(ctx->enc_req), sizeof(struct sec_bd_info));
	else
		memcpy(msg, &(ctx->dec_req), sizeof(struct sec_bd_info));
	...
	*(uint64_t *)(&sec_req->req.mac_addr) = (uint64_t)sec_req;
	...
	if (sec_req->req.de){
	    sec_req->req.cipher_destin_addr = ADDR_L(sec_req->sec_udata.dma_out);
	    sec_req->req.cipher_destin_addr_hi = ADDR_H(sec_req->sec_udata.dma_out);
	} else {
	    sec_req->req.cipher_destin_addr = ADDR_L(sec_req->sec_udata.dma_in);
	    sec_req->req.cipher_destin_addr_hi = ADDR_H(sec_req->sec_udata.dma_in);
	}
...
int hisi_sec_algs_register(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(sec_algs); i++)
		sec_algs[i].cra_flags = CRYPTO_ALG_TYPE_ABLKCIPHER | CRYPTO_ALG_ASYNC;
