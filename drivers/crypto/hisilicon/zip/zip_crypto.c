...
static u8 hisi_zip_get_alg_type(struct crypto_tfm *tfm)
{
	const char *alg_name = crypto_tfm_alg_name(tfm);
	u8 i;

	for (i = 1; strcmp("NULL", hisi_zip_alg_type[i]); i++) {
		if (!strcmp(alg_name, hisi_zip_alg_type[i]))
			return i;
	}
...
/* fix me: let's allocate one buffer now, may have problem in async case */
...
/* fix me: same as allocating function */
...
static int hisi_zip_create_qp(struct qm_info *qm,
					  struct hisi_zip_qp_ctx *ctx,
...
static int hisi_zip_alloc_comp_ctx(struct crypto_tfm *tfm)
{
	...
	struct qm_info *qm;
	int ret, i, j;
	u8 req_type = hisi_zip_get_alg_type(tfm);
	...
	for (i = 0; i < 2; i++) {
		/* it is just happen that 0 is compress, 1 is decompress on
		 * alg_type
		 */
...
/* fix me: only need in crypto sync API */
